...
if not open('question_word.train','r+'):
    ...
    with open('question_word.train','w') as word_file:
...
if not open('answer_word.train','r+'):
    ...
    with open('answer_word.train','w') as word_file:
        for key in trainKeyOrder:
...
if not open('choices_word.train','r+'):
    ...
    with open('choices_word.train','w') as word_file:
        for key in trainKeyOrder:
...
if not open('question_word.test','r+'):
    ...
    with open('question_word.test','w') as word_file:
...
if not open('choices_word.test','r+'):
    ...
    with open('choices_word.test','w') as word_file:
        for key in testKeyOrder:
            for index in choices_dict[key]:
                word_file.write(index + '\n')

#make a file of total words to train word-embedding model
with open('word_embed.txt','w') as word_file:
    for line in open('question_word.train','r+'):
        word_file.write(line)
    for line in open('choices_word.train','r+'):
        word_file.write(line)
    for line in open('question_word.test','r+'):
        word_file.write(line)
    for line in open('choices_word.test','r+'):
        word_file.write(line)

logging.basicConfig(format='%(asctime)s : %(threadName)s : %(levelname)s : %(message)s', level=logging.INFO)
logging.info("running %s" % " ".join(sys.argv))

input_file = 'word_embed.txt'
#make model
word_model = Word2Vec(LineSentence(input_file), size=400, window=5, sg=0, workers=8)
#save model
word_model.save(input_file + '.model')
#save word vectors
word_model.save_word2vec_format(input_file + '.vec')

sent_file = 'question_word.train'
...
sent_file = 'answer_word.train'
sent_model = Sent2Vec(LineSentence(sent_file), model_file=input_file + '.model')
sent_model.save_sent2vec_format(sent_file + '.vec')

sent_file = 'choices_word.train'
sent_model = Sent2Vec(LineSentence(sent_file), model_file=input_file + '.model')
sent_model.save_sent2vec_format(sent_file + '.vec')

sent_file = 'question_word.test'
sent_model = Sent2Vec(LineSentence(sent_file), model_file=input_file + '.model')
sent_model.save_sent2vec_format(sent_file + '.vec')

sent_file = 'choices_word.test'
sent_model = Sent2Vec(LineSentence(sent_file), model_file=input_file + '.model')
sent_model.save_sent2vec_format(sent_file + '.vec')
